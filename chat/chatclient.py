...
class ChatClient():#聊天室的客户端类，实现客户端功能，包括创建客户端socket,连接服务器，收发服务器端和其他客户端的数据
    ...
    def receivemessage(self):
        while True:
            ...
            if self.client_socket in readlist:
                try:#从服务器接收数据，数据buffer为4096
                    print(self.client_socket.recv(4096).decode('utf-8'))
                except socket.error as err:
                    print('连接错误...')
                    exit()
    def sendmessage(self):#发送数据，将客户端用户输入的信息发送出去
        while True:
            try:
                data = input()
...
if __name__ == "__main__":#为了便于测试，这里使用本机hostname
...
#如果打开的客户端关闭了，服务器端就会显示出 该用户离开了聊天室。
#用户名被强制输入，在客户端输入用户名之后就可以进入聊天室。

#使用不同计算机连接服务器（服务器的IP地址和端口号）。