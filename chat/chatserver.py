...
class ChatServer():
    """聊天室的服务器类
    实现服务端的功能
    包括创建服务端socket,连接客户端，收发客户端的数据"""
    def __init__(self,host,port,numOfClients): #初始化服务器
        self.HOST = host
        self.PORT = port
    ...
    def run(self):#响应客户端的连接和传输数据
        while True:#如果只是服务器开启，36000秒之内没有客户端连接，则会超时关闭
            readlist,writelist,errorlist = select.select(self.socket_list,[],[],36000)#当读入列表readlist中没有可读信息时，
            # 即没有用户接入聊天室，则会退出服务器
            if not readlist:
                print('没有用户连接，聊天室关闭...')
            ...
            for client_socket in readlist:
                if client_socket is self.server_socket:
                    ...
                    disconnection = False
                    try:#接受客户端data,数据buffer大小设置为4096
                        data = client_socket.recv(4096).decode('utf-8')
                        data = self.client_names[client_socket] + ':' + data
                    except socket.error as err:#客户端连接异常，则认为该用户已经离线即离开聊天室
                        data = self.client_names[client_socket] + '离开聊天室。'
                        disconnection = True

                    if disconnection:#如果用户离开聊天室，则从对应的客户端从读入列表readlist中移除
